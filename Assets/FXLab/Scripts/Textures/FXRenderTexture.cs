...
public class FXRenderTexture : ScriptableObject
{
	...
	public void SetShaderData(Material target, string name)
    {
        ...
        if (SizeMode != FXRenderTextureSizeMode.ImageEffect)
        {
            area.xMin += 0.5f;
            area.yMin += 0.5f;
            area.xMax -= 0.5f;
        ...
        SetShaderDataValues(target, name, Registration.Chart.Data.Texture, Registration.Chart.Data.TexelSize, normalizedArea);

        if (!target)
            LastSetRenderTextures[name] = this;
    }

    public static void SetShaderDataValues(Material target, string name, Texture texture, Vector2 texelSize, Vector4 normalizedArea)
    {
