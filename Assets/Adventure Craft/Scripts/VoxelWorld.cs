...
//				}
		...
		{
				regions.Remove (region.hashString ());
				loadedRegions -= 1;
		}
		public Region createRegion (int x, int y, int z, bool isAsync)
		{
				...
				loadedRegions += 1;
				RegionWatcher watcher = new RegionWatcher (region, x, y, z);
				unloaderQueue.Add (watcher);
				if (isAsync) {
						ThreadPool.QueueUserWorkItem (new WaitCallback (region.create), null);
						return null;
				} else {
				...
				if (useDisk) {
						foreach (KeyValuePair<string,Region> kvp in regions) {
								...
								if (kvp.Value.isDirty) {
										Debug.Log ("Region " + kvp.Value.hashString () + " is dirty!");
		...
		private class RegionWatcher : IComparable<RegionWatcher>
		{
				public float timeSinceActive;
				public int x, y, z;
				public Region region;

				public RegionWatcher (Region inRegion, int xi, int yi, int zi)
				{
						region = inRegion;
						x = xi;
						y = yi;
				...
				public int CompareTo (RegionWatcher other)
				{
						return (int)(this.timeSinceActive - other.timeSinceActive);
				}
		}
		
		public int unloadBlockDistance = 600;
		IEnumerator UnloadRegions ()
		{
				for (;;) {
						Debug.Log ("Checking Regions");
						RegionWatcher regionWatcher = unloaderQueue.Peek ();
						while (regionWatcher != null) {
								if (Time.realtimeSinceStartup - regionWatcher.timeSinceActive > 30) {
										unloaderQueue.Remove ();
										float distance = Vector3.Distance (clientRenderer.player.transform.position, 
											new Vector3 (regionWatcher.x * Region.regionXZ, 
												regionWatcher.y * Region.regionY, 
												regionWatcher.z * Region.regionXZ));
										if (Math.Abs (distance) > unloadBlockDistance) {
												Debug.Log ("Region:" + regionWatcher.x + "," + regionWatcher.y + "," + regionWatcher.z + " unloading.");
												destroyRegion (regionWatcher.region);
										} else {
												//update time stamp and put back in queue.
												regionWatcher.timeSinceActive += 30;
												unloaderQueue.Add (regionWatcher);
										}
										regionWatcher = unloaderQueue.Peek ();
								} else {
