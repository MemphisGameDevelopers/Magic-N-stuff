// optimized version of the original "MeshCombineUtility" which does not create a second uv
// we do not need a 2nd uv as lightmapping is not supported on foliage.
...
public class MeshCombineUtilityAFS {
	...
	public static Mesh Combine (MeshInstance[] combines, bool bakeGroundLightingGrass, bool bakeGroundLightingFoliage, float randomBrightness, float randomPulse, float randomBending, float randomFluttering, Color HealthyColor, Color DryColor, float NoiseSpread, bool bakeScale, bool simplyCombine, float NoiseSpreadFoliage)
	{
		int vertexCount = 0;
		int triangleCount = 0;
		...
		foreach( MeshInstance combine in combines )
		{
			if (combine.mesh) {
				...
				// or simply add random color to create more variety (r,a and b) and bake scale
					...
					CopyColors(combine.mesh.vertexCount, combine.mesh.colors, colors, ref offset, combine.scale, bakeScale, combine.pivot, NoiseSpreadFoliage, randomPulse, randomFluttering, randomBrightness, randomBending);
...
// add random colors (r, g b, a) to create more variety (foliage shader)
	static void CopyColors (int vertexcount, Color[] src, Color[] dst, ref int offset, float scale, bool bakeScale, Vector3 pivot, float NoiseSpread, float randomPulse, float randomFluttering, float randomBrightness, float randomBending)
	{
		for (int i=0;i<src.Length;i++) {
