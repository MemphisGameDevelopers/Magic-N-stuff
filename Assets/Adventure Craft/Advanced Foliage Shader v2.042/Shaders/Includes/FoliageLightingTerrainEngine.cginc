...
fixed _AfsTranslucencyViewDependency;
half _AfsShadowStrength;
...
inline half4 LightingLeafTerrainEngine (LeafSurfaceOutput s, half3 lightDir, half3 viewDir, half atten)
{
	...
	backContrib = lerp(saturate(-nl), backContrib, _AfsTranslucencyViewDependency);
	
	fixed3 translucencyColor = backContrib * s.Translucency * _AfsTranslucencyColor;
	...
	c.rgb *= lerp(2, atten * 2, _AfsShadowStrength);
	#else
	c.rgb *= 2*atten;
	#endif
