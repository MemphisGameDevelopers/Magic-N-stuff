Shader "AFS_v2/Advanced Foliage Shader Groundlighting Atlas" {
Properties {
	_Cutoff ("Alpha cutoff", Range(0,1)) = 0.3
	_MainTex ("Base (RGB) Alpha (A)", 2D) = "white" {}
	_Shininess("Shininess", Range(0.03,1)) = 0.2
	
	// this property aloso identifies the shader: do not rename or delete it
	_GroundLightingAttunation ("GroundLighting Attenuation", Range(0,5)) = 1
...
SubShader { 
	Tags {
		...
		"RenderType"="AtsFoliage"
	}
	LOD 200
	//Offset -1,0

CGPROGRAM
#pragma surface surf TreeLeaf alphatest:_Cutoff vertex:CustomBendingGroundlighting nolightmap nodirlightmap
#pragma exclude_renderers flash
#pragma glsl_no_auto_normalization

#include "Includes/TreeGround.cginc"
#include "TerrainEngine.cginc"
#include "Includes/CustomBending.cginc"

// needed by Groundlighting
#pragma target 3.0
...
struct Input {
	...
	float2 uv_Second : texcoord1; // compressed ground normal ist store in the 2nd uv set
};
...
void surf (Input IN, inout LeafSurfaceOutput o) {
	...
	// add "vertex lighting" which is stored in vertex.color.r
	o.GroundLighting = IN.color.r;
}
ENDCG
