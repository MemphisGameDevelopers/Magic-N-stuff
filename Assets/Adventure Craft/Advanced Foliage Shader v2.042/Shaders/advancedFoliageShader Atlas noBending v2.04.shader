Shader "AFS_v2/Advanced Foliage Shader Atlas noBending" {
Properties {
	_Cutoff ("Alpha cutoff", Range(0,1)) = 0.3
	_MainTex ("Base (RGB) Alpha (A)", 2D) = "white" {}
	_Shininess("Shininess", Range(0,1)) = 0.2
	
	_Color ("Main Color (unused but needed)", Color) = (1,1,1,1) 
...
CGPROGRAM
#pragma surface surf TreeLeaf alphatest:_Cutoff nolightmap nodirlightmap
...
void surf (Input IN, inout LeafSurfaceOutput o) {
	...
	o.Albedo = c.rgb * IN.color.a * _Color.rgb;
...
ENDCG
	...
	Pass {
		...
		half4 frag_surf (v2f_surf IN) : COLOR {
			...
			// as we use blue in order to store spec values we skip all this
...
SubShader {
	Tags {
		"IgnoreProjector"="True"
		"RenderType"="AtsFoliageNoBending"
	}
...
SubShader {
	Tags { "RenderType"="AtsFoliageNoBending" }
