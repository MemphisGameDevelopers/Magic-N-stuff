Shader "AFS_v2/AFS Tree Creator Leafs Optimized ShadowFade" {
Properties {
...
CGPROGRAM
#pragma surface surf TreeLeaf alphatest:_Cutoff vertex:MyTerrainTreeVertLeaf nolightmap
...
//	add billboardShadows
	...
	o.Albedo = c.rgb * IN.color.a * _Color.rgb * lerp( shadowColor, fixed3(1,1,1), IN.color.r); // * IN.color.b);
	#else
	o.Albedo = c.rgb * IN.color.a * _Color.rgb * lerp( shadowColor, fixed3(1,1,1), IN.color.b); // * IN.color.b);
	#endif
	
//	store fadeState
	#ifdef BILLBOARD_SHADOWS
	// do not fade in real time shadows before the transition is finished
	// as the shadow caster does not fade but the shadow receiver does
	o.ShadowCutOff = IN.color.r;
	#else
...
//			dissolve
		...
		v2f_surf vert_surf (appdata_full v) {
			v2f_surf o;
			MyTerrainTreeVertLeafShadowCaster (v);
			#ifdef TREE_SHADOW_DISSOLVE
//			dissolve
			o.fade  = v.color.b;
			#endif
...
Dependency "BillboardShader" = "Hidden/Nature/AFS Tree Creator Leafs Rendertex"
}
